...
from laser.matrix_utils import do_low_rank, sorted_mat, prune,do_ksvd,do_low_pass,do_high_pass
...
class LLAMA2Laser(AbstractLaser):
    ...
    @staticmethod
    def get_edited_model(model, lname, lnum, rate, threshold, intervention="do_low_pass", logger=None, in_place=True):
        ...
        for name, param in model.named_parameters():
            ...
            elif intervention == 'ksvd':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_ksvd(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
            elif intervention == 'do_low_pass':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_low_pass(mat_analysis_tensor.type(torch.float32), rate, threshold)
            elif intervention == 'do_high_pass':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_high_pass(mat_analysis_tensor.type(torch.float32), rate, threshold)
