...
from laser.matrix_utils import do_low_rank, sorted_mat, prune,do_low_pass,do_high_pass,do_quant
    ...
    def get_edited_model(model, lname, lnum, rate, intervention="rank-reduction", logger=None, in_place=True):
        ...
        for name, param in model.named_parameters():
            ...
            if intervention == 'dropout':
                ...
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_quant(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
            elif intervention == 'lowpass':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_low_pass(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
                
            elif intervention == 'highpass':
                # Do rank reduction
                mat_analysis_tensor = deepcopy(param)
                mat_analysis = do_high_pass(mat_analysis_tensor.type(torch.float32), (10 - rate) * 0.1)
