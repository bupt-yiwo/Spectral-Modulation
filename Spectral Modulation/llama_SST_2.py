...
from dataset_utils.SST_2 import SST
...
class LlamaExperiment:
    ...
    def intervene(self, model, tokenizer, questions, answers , args):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(0, dataset_size)):
                ...
                answer_ix = int(answers[i])
                #{'+', '!', '?', ':', "'", '.', '-', ',', ')', ';'}
                if question.strip().endswith(".") or question.strip().endswith("?")or question.strip().endswith("!")or question.strip().endswith(";"):
                    prompted_question = "Consider the following sentence: " + \
                                        question.strip() + " Is the sentiment expressed in this comment positive or negative? The result is "
                else:
                    prompted_question =  "Consider the following comment: " + \
                                        question.strip() + ". Is the sentiment expressed in this comment positive or negative? The result is "
...
parser.add_argument('--home_dir', type=str,
                    default="./results/SST_2/vicuna_7b_results",
...
dataset_util = SST()
questions, answers = dataset_util.get_dataset(logger)
