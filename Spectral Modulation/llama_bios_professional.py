...
class LlamaExperiment:
    ...
    def intervene(self, model, tokenizer, dataset, args):
        ...
        acc_all = np.mean([1.0 if prediction["correct"] else 0.0 for prediction in predictions]) * 100.0
        return acc_all
...
parser.add_argument('--home_dir', type=str,
                    default="./results/bios_professional/vicuna_7b_results",
...
acc = experiment.intervene(model=model,
                        tokenizer=tokenizer,
                        dataset=dataset,
                        args=args)
